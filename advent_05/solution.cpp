...
Pipe::Pipe(std::pair<int, int> from, std::pair<int, int> to)
{
    ...
    {
        throw std::invalid_argument("pipe is not horizontal nor vertical");
    }
}
...
std::vector<Pipe> readPipes(std::istream& is)
{
    ...
    for (std::string line; std::getline(is, line, '\n');)
    {
        ...
        std::cout << "from: " << from << " is : x=" << from_x << " y=" << from_y << std::endl;
        std::cout << "to: " << to << " is : x=" << to_x << " y=" << to_y << std::endl;
        try 
        {
